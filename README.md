...
- The Peltier elements I used did not consume the wattage according to the specs, so I assume them to be of inferior quality, and therefore will not provide links to where I got them. They do the job, but if they would work according to specs, they should provide even greater cooling.
- This project was not planned, but rather built as a proof of concept to see if the concept works at all, so there are many things that can be improved or changed, e.g. less glueing.
...
- Connect 2 (red) AWG 16 cables for the Peltier elements to one of the +24V connectors of the 220V voltage adapter and let them hang loose for now
- Connect 2 (black) AWG 16 cables for the Peltier elements to one of the GND connectors of the 220V voltage adapter and let them hang loose for now
...
- Connect the three V+ and three GND output cables of the low voltage adapters with the 6 2xWAGO connectors (one connection per WAGO)
...
| Set 3V, 5V or 12V for the LED lighting<br> (For the LED lights linked in the parts list above, please use 3V. The picture was taken before that was installed) | <img src="images/assembly/2c_measure.jpg" width="500"> |

## 3. Case Cover - Under Side

__NOTE:__ All wires should have their ends soldered before putting them into the WAGO connectors. It's not mandatory, but prevents problems of bad connections
...
- Leave the 2 AWG10 wires from the other connection of the Light and Heating switches hanging loose (they'll be connected to the case base later)
...
__Note:__ Usually the side with the printing is the cooling side of the Peltier elements. But double check if that is true for the ones you use.
...
| - Screw the LED spot lights onto the carrier and feed their cables through the small opening of the box  | <img src="images/assembly/7b_light.jpg" width="400"> |
| - Attach the box to the cooling tower<br> - Fixate it with screws if necessary<br> The height of the LED spot lights can be adjusted by moving the LED carrier and fixating it with the screws from behind | <img src="images/assembly/7c_light.jpg" width="400"> |
...
- Connect the V+ wires from the LEDs and heating element with the corresponding WAGO connectors<br>(Please note that in the picture, the GND cable wrongly is connected. The cable is supposed to be red, not black)
- Leave the GND wires from the LEDs and heating element hanging loose
<img src="images/assembly/7a_cover_underside.jpg" width="800"> 

## 10. Connect Cover to Bottom
- Connect the loose cables from the top on the cover with the corresponding connectors in the base
...
- The fan will start turning and the voltage displays will show the voltage they deliver to the peltier elements
- Adjust the voltage output of the voltage adapters on the case cover for the peltier elements using a screw driver and rotating the voltage adjustment clockwise. These values worked well for me:
  - Upper Peltier: 5V
  - Lower Peltier: 10.5V
- Make sure that you open up the current fully for the adapters
  - Screw the current adjustment clockwise as far as it goes
